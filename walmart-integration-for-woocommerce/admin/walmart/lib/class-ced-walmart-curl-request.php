...
class Ced_Walmart_Curl_Request {
	 ...
	 * @since 1.0.0
	 */
	public function ced_walmart_get_request( $action = '', $parameters = '', $query_args = array() ) {
		if ( empty( $action ) ) {
			return;
		}
	 ...
	 * @since 1.0.0
	 */
	public function get_additional_headers( $action = '' ) {
		$headers = array();
		if ( 'token' == $action ) {
			...
			if ( 'sandbox' == $this->environment ) {
				$headers[] = 'HOST: sandbox.walmartapis.com';
			} else {
	...
	public function load_depenedency() {
		...
		if ( file_exists( $file ) ) {
			include_once $file;
			$this->ced_walmart_config_instance = Ced_Walmart_Config::get_instance();
		}
	}
}
