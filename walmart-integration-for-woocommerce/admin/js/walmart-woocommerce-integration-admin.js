(function( $ ) {
	...
	$( document ).on(
		'click',
		'#ced_walmart_update_api_keys',
		function(){

			can_ajax = true;
			$( '.ced_walmart_required_data' ).each(
				function() {
					if ( $( this ).val() == '' ) {
			...
			process_configuration_details( can_ajax , 'update' );
		}
	);

	$( document ).on(
		'click',
		'#ced_walmart_validate_api_keys',
		function(){
			process_configuration_details( true , 'validate' );
		}
	);

	function process_configuration_details(can_ajax,operation) {
		if ( can_ajax ) {
			$( '.ced_walmart_loader' ).show();
			var client_id , client_secret , environment , wfs;
			client_id     = $( '#ced_walmart_client_id' ).val();
			client_secret = $( '#ced_walmart_client_secret' ).val();
			environment   = $( '#ced_walmart_environment' ).val();
			wfs           = "";
			if ( $( '#ced_walmart_wfs' ).is( ':checked' ) ) {
				wfs = "on";
			}
			...
			$.ajax(
				{
					...
					data :{
						...
						action : 'ced_walmart_process_api_keys',
						operation : operation,
	...
	$( document ).on(
		'click',
		'.ced_walmart_parent_element',
		function(){
			if ($( this ).find( '.ced_walmart_instruction_icon' ).hasClass( "dashicons-arrow-down-alt2" )) {
				$( this ).find( '.ced_walmart_instruction_icon' ).removeClass( "dashicons-arrow-down-alt2" );
				$( this ).find( '.ced_walmart_instruction_icon' ).addClass( "dashicons-arrow-up-alt2" );
			} else if ($( this ).find( '.ced_walmart_instruction_icon' ).hasClass( "dashicons-arrow-up-alt2" )) {
				$( this ).find( '.ced_walmart_instruction_icon' ).addClass( "dashicons-arrow-down-alt2" );
				$( this ).find( '.ced_walmart_instruction_icon' ).removeClass( "dashicons-arrow-up-alt2" );
			}
			$( this ).next( '.ced_walmart_child_element' ).slideToggle( 'slow' );
		}
	);

	$( document ).on(
		'click',
		'.ced_walmart_global_tab_label',
		function(){
			var tab = $( this ).data( 'tab' );
			$( '.ced_walmart_global_tab_label' ).removeClass( 'active' );
	...
	$( document ).on(
		'click',
		'.ced_walmart_add_key_feature',
		function(){
			var count_keyfeature = $( document ).find( '.key_features' ).length;
			var keyfeature_html  = '';
			keyfeature_html     += '<tr class="form-field key_features">';
			keyfeature_html     += '<input type="hidden" name="ced_walmart_required_common[]" value="global_keyfeatures_' + ( count_keyfeature + 1 ) + '">';
			keyfeature_html     += '<td><input class="short" style="" name="global_keyfeatures[]" id="" value="" placeholder="" type="text"></td>';
			keyfeature_html     += '<td><input type="button" class="button button-primary ced_walmart_add_key_feature" value="+" ></td>';
			keyfeature_html     += '</tr>';
			$( keyfeature_html ).insertAfter( 'tr.key_features:last' );
			( this ).remove();
		}
	);
			...
			$.ajax(
				{
					...
					data : {
						ajax_nonce : ajax_nonce,
						action : 'ced_walmart_list_per_page',
						per_page : per_page,
					},
					type : 'POST',
					success: function( response ) {
	...
	$( document ).on(
		...
		function(){
			...
			for (var i = 0; i < walmart_order_item_count;  i++) {
				var order_lineNumber     = jQuery( "#lineNumber_" + i ).val();
				var product_sku          = jQuery( "#sku_" + i ).val();
				var requested_quantity   = jQuery( "#qty_" + i ).val();
				var cancel_quantity      = jQuery( "#can_" + i ).val();
				var ship_quantity        = jQuery( "#ship_" + i ).val();
				var avail_check_quantity = jQuery( "#avail_" + i ).val();
			...
			jQuery( ".ced_walmart_loader" ).show();
	...
	// For walmart fulfillment Service category Selection

	$( document ).on(
		'change',
		'#ced_walmart_category_wfs',
		function(){
			let value = $( this ).val();
			let catId = $( this ).attr( "data-store-category-id" );
			$( '.ced_walmart_loader' ).show();
			$.ajax(
				{
					...
					data : {
						ajax_nonce : ajax_nonce,
						action : 'ced_walmart_save_cat_for_wfs',
						value:value,
						catId:catId,
					},
					type : 'POST',
	...
	// For walmart fulfillment Service Subcategory Selection

	$( document ).on(
		'change',
		'#ced_walmart_subcategory_wfs',
		function(){
			let value = $( this ).val();
			let catId = $( this ).attr( "data-store-category-id" );
			$( '.ced_walmart_loader' ).show();
			$.ajax(
				{
					...
					data : {
						ajax_nonce : ajax_nonce,
						action : 'ced_walmart_save_subcat_for_wfs',
						value:value,
						catId:catId,
					},
					type : 'POST',
	...
	// For walmart fulfillment Service category Selection for New Item

	$( document ).on(
		'change',
		'#ced_walmart_category_new_item_wfs',
		function(){
			let value = $( this ).val();
			let catId = $( this ).attr( "data-store-category-id" );
			$( '.ced_walmart_loader' ).show();
			$.ajax(
				{
					...
					data : {
						ajax_nonce : ajax_nonce,
						action : 'ced_walmart_save_cat_for_new_item_wfs',
						value:value,
						catId:catId,
					},
					type : 'POST',
	...
	// For walmart fulfillment Service Subcategory Selection for New Item

	$( document ).on(
		'change',
		'#ced_walmart_subcategory_new_item_wfs',
		function(){
			let value = $( this ).val();
			let catId = $( this ).attr( "data-store-category-id" );
			$( '.ced_walmart_loader' ).show();
			$.ajax(
				{
					...
					data : {
						ajax_nonce : ajax_nonce,
						action : 'ced_walmart_save_subcat_for_new_item_wfs',
						value:value,
						catId:catId,
					},
					type : 'POST',
