...
class Ced_Walmart_Wfs_Profiles_List extends WP_List_Table {
	...
	public function prepare_items() {
		global $wpdb;
		/** Get per page number to list wfs profile
		 *
		 * @since 1.0.0
		 */
	...
	public function ced_walmart_get_profiles( $per_page = 10, $page_number = 1 ) {
		$result = get_option( 'ced_mapped_wfs_cat' );
	...
	public function column_wfs_cat_name( $ced_walmart_profile_details ) {
		...
		$actions['edit']    = sprintf( '<a href="?page=%s&section=%s&tab=%s&wfs_profile_id=%s&panel=edit_wfs">Edit</a>', $request_page, 'wfs', 'wfs_profile_list', $format_cat_for_url );
	...
	public function process_bulk_action() {
		...
		if ( isset( $_POST['action'] ) && 'bulk-delete' == $_POST['action'] || isset( $_POST['action2'] ) && 'bulk-delete' == $_POST['action2'] ) {
			...
			foreach ( $profile_ids as $index => $profile_id ) {
				$ced_walmart_profile_details = get_option( 'ced_mapped_wfs_cat' );
				...
				foreach ( $ced_walmart_profile_details['profile'] as $key => $value ) {
					if ( $key == $profile_id ) {
						foreach ( $value['woo_cat'] as $key_id => $value_id ) {
							delete_term_meta( $value_id, 'ced_walmart_wfs_category' );
						}
						unset( $ced_walmart_profile_details['profile'][ $profile_id ] );
						if ( empty( $ced_walmart_profile_details['profile'] ) ) {
							delete_option( 'ced_mapped_wfs_cat' );
						} else {
							update_option( 'ced_mapped_wfs_cat', json_encode( $ced_walmart_profile_details ), 1 );
						}
					}
				}
			}
			$redirect_url = get_admin_url() . 'admin.php?page=ced_walmart&section=wfs&tab=wfs_profile_list';
			wp_redirect( $redirect_url );
			exit;
		} elseif ( isset( $_GET['panel'] ) && 'edit_wfs' == $_GET['panel'] ) {
			$file = CED_WALMART_DIRPATH . 'admin/partials/ced-walmart-wfs-profile-edit.php';
			if ( file_exists( $file ) ) {
				include_once $file;
			}
		}
