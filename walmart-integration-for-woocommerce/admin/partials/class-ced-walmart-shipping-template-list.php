...
class Ced_Walmart_Shipping_Template_List extends WP_List_Table {
	...
	public function get_count() {
		$ced_walmart_shipping_templates = json_decode( get_option( 'ced_walmart_shipping_templates' . wifw_environment() ), 1 );
...
<div class="ced_walmart_heading">
					...
					<div class="ced_walmart_child_element default_modal">
						<ul type="disc">
						<li type=""><?php esc_html_e( 'Create Your Own Shipping Template' ); ?></li>
						<li type=""><?php esc_html_e( 'You can create maximum of 60 shipping templates, Including Standard, Paid Standard & Freight shipping templates.' ); ?></li>
						<li type=""><?php esc_html_e( 'Do not see all of your shipping templates? Just click on Fetch -> Shipping Templates to fetch all the newly `Created` shipping templates from Walmart.com.' ); ?></li>
			</ul>
					</div>
				</div>
		<div class="ced_walmart_wrap ced_walmart_wrap_extn">
				...
				<div id="ced_shipping_types">
					<a  href="?page=ced_walmart&section=shipping_template&panel=create_template&type=standard" type="button" class="ced_shipping_type" name="">
						Standard Template
					</a>

					<a  href="?page=ced_walmart&section=shipping_template&panel=create_template&type=paidstandard" type="button" class="ced_shipping_type" name="">
		...
		if ( isset( $_GET['panel'] ) && 'delete' == $_GET['panel'] ) {
			...
			if ( $template_id ) {
				...
				if ( isset( $response['id'] ) ) {
					...
					wp_redirect( 'admin.php?page=ced_walmart&section=shipping_template' );
				}
