...
$_per_page                        = 10;
$page_no                          = '';
$ced_walmart_category_wfs_file    = CED_WALMART_DIRPATH . 'admin/walmart/lib/json-schema/schema/wfs_convert/ConvertSpecRequest.json';
...
if ( file_exists( $ced_walmart_category_wfs_file ) ) {
	...
	$wfs_cat_search         = $walmart_wfs_categories['properties']['SupplierItem']['items']['properties']['Visible']['properties'];
}
				...
				<tbody>
					...
					foreach ( $woo_store_categories as $key => $value ) {
						...
						<tr class="ced_walmart_store_category" id="<?php echo esc_attr( 'ced_walmart_store_category_' . $value->term_id ); ?>">
							...
							<td colspan="4">
								<select class="ced_walmart_category_wfs ced_walmart_select_category_wfs select2 ced_walmart_select2 select_boxes_cat_wfs_map" id="ced_walmart_category_wfs" name="ced_walmart_category_wfs"  data-store-category-id="<?php echo esc_attr( $value->term_id ); ?>" >
									<option value="">--<?php esc_html_e( 'Select', 'walmart-woocommerce-integration' ); ?>--</option>
									<?php
									$selected_wfs_categories = get_term_meta( $value->term_id, 'ced_walmart_wfs_category', true );
							...
							<td colspan="4">
								<select class="ced_walmart_subcategory_wfs ced_walmart_select_subcategory_wfs select2 ced_walmart_select2 select_boxes_subcat_wfs_map" id="ced_walmart_subcategory_wfs" name="ced_walmart_subcategory_wfs"  data-store-category-id="<?php echo esc_attr( $value->term_id ); ?>" >
									<option value="">--<?php esc_html_e( 'Select', 'walmart-woocommerce-integration' ); ?>--</option>
									<?php
									$selected_subcategories = get_term_meta( $value->term_id, 'ced_walmart_wfs_subcategory', true );
