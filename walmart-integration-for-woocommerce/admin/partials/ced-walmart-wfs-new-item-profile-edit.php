...
$profile_id                    = isset( $_GET['wfs_new_item_profile_list'] ) ? sanitize_text_field( wp_unslash( $_GET['wfs_new_item_profile_list'] ) ) : '';
...
if ( isset( $_POST['add_meta_keys'] ) || isset( $_POST['ced_walmart_profile_save_button'] ) ) {
	...
	$ced_walmart_wfs_profile_details = get_option( 'ced_mapped_wfs_new_item_cat' );
	...
	if ( $profile_id ) {
		...
		update_option( 'ced_mapped_wfs_new_item_cat', json_encode( $ced_walmart_wfs_profile_details ) );
	}
}
					...
					<tbody>
						...
						<tr>
							...
							if ( file_exists( $file_product_fields ) ) {
								...
								$schema               = CED_WALMART_DIRPATH . 'admin/walmart/lib/json-schema/schema/wfs_convert/MP_WFS_ITEM_SPEC.json';
								...
								$wfs_cat_search     = $schema['properties']['MPItem']['items']['properties']['Visible']['properties'];
								...
								foreach ( $wfs_cat_search as $key => $value ) {
									if ( $key == $get_profile_name ) {
										...
										$wfs_orderable_search = $schema['properties']['MPItem']['items']['properties']['Orderable']['properties'];
										$wfs_tradeItem_search = $schema['properties']['MPItem']['items']['properties']['TradeItem']['properties'];
										update_option( 'ced_walmart_wfs_new_item_orderable_' . $get_profile_name, json_encode( $wfs_orderable_search ) );
										update_option( 'ced_walmart_wfs_new_item_visible_' . $get_profile_name, json_encode( $value['properties'] ) );
										update_option( 'ced_walmart_wfs_new_item_tradeItem_' . $get_profile_name, json_encode( $wfs_tradeItem_search ) );
										$wfs_orderable_required  = $schema['properties']['MPItem']['items']['properties']['Orderable']['required'];
										$wfs_tradeItem_required  = $schema['properties']['MPItem']['items']['properties']['TradeItem']['required'];
										...
										foreach ( $merged_preapre as $attributesKey => $attributesValue ) {
											...
											if ( 'object' == $attributesValue['type'] ) {
												foreach ( $attributesValue['properties'] as $objectkey => $objectvalue ) {
													...
													if ( ! isset( $objectvalue['enum'] ) && ! isset( $objectvalue['items']['enum'] ) ) {
														...
														} elseif ( isset( $objectvalue['type'] ) && 'array' == $objectvalue['type'] ) {
															$type_wfs   = 'array';
															$input_type = 'text';
														}
											...
											if ( ! isset( $attributesValue['enum'] ) && ! isset( $attributesValue['items']['enum'] ) && ! isset( $attributesValue['items']['properties'] ) ) {
												...
												if ( 'integer' == $attributesValue['type'] || 'number' == $attributesValue['type'] ) {
													...
													$type_wfs   = 'datetime-local';
													$input_type = 'datetime-local';
												} elseif ( isset( $attributesValue['type'] ) && 'array' == $attributesValue['type'] ) {
													$type_wfs   = 'array';
													$input_type = 'text';
												}
									...
									update_option( 'ced_wfs_new_item_type_attribute_' . $get_profile_name, json_encode( $type_wfs_attribute ) );
