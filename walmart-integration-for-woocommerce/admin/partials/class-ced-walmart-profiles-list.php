...
class Ced_Walmart_Profiles_List extends WP_List_Table {
	 ...
	 * @param array $ced_walmart_profile_details Profile Data.
	 */
	public function column_cat_name( $ced_walmart_profile_details ) {
		$title              = '<strong>' . $ced_walmart_profile_details['cat_name'] . '</strong>';
		...
		$format_cat_for_url = str_replace( ' & ', ' and ', $ced_walmart_profile_details['cat_name'] );
		$actions['edit']    = sprintf( '<a href="?page=%s&section=%s&profile_id=%s&panel=edit">Edit</a>', $request_page, 'profiles', $format_cat_for_url );
	...
	public function get_columns() {
		$columns = array(
			...
			'cat_name'       => __( 'Walmart Profile Name', 'walmart-woocommerce-integration' ),
	...
	public function process_bulk_action() {
		...
		if ( isset( $_POST['action'] ) && 'bulk-delete' == $_POST['action'] || isset( $_POST['action2'] ) && 'bulk-delete' == $_POST['action2'] ) {
			...
			$redirect_url = get_admin_url() . 'admin.php?page=ced_walmart&section=profiles';
