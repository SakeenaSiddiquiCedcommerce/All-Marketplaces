...
function get_navigation_menus() {
	...
	$navigation_menus = array(
		'configuration'     => admin_url( 'admin.php?page=ced_walmart' ),
		'global_settings'   => admin_url( 'admin.php?page=ced_walmart&section=global_settings' ),
		'category_mapping'  => admin_url( 'admin.php?page=ced_walmart&section=category_mapping' ),
		'profiles'          => admin_url( 'admin.php?page=ced_walmart&section=profiles' ),
		'products'          => admin_url( 'admin.php?page=ced_walmart&section=products' ),
		'orders'            => admin_url( 'admin.php?page=ced_walmart&section=orders' ),
		'shipping_template' => admin_url( 'admin.php?page=ced_walmart&section=shipping_template' ),
		'insights'          => admin_url( 'admin.php?page=ced_walmart&section=insights' ),
		'feeds'             => admin_url( 'admin.php?page=ced_walmart&section=feeds' ),
	);
	if ( 'on' == $wfs ) {
		$navigation_menus['wfs'] = admin_url( 'admin.php?page=ced_walmart&section=wfs' );
	}
	/** Get walmart menus
	 *
	 * @since 1.0.0
 ...
 * @since 1.0.0
 */
function ced_walmart_render_html( $meta_keys_to_be_displayed = array(), $added_meta_keys = array() ) {
	$html  = '';
	...
	if ( isset( $meta_keys_to_be_displayed ) && is_array( $meta_keys_to_be_displayed ) && ! empty( $meta_keys_to_be_displayed ) ) {
		...
		foreach ( $meta_keys_to_be_displayed as $meta_key => $meta_data ) {
			...
			if ( 0 == $counter ) {
				...
				$html .= '<tbody style="' . esc_attr( $display ) . '" class="ced_walmart_metakey_list_' . $break_point . '  			ced_walmart_metakey_body">';
				$html .= '<tr><td colspan="3"><label>CHECK THE CUSTOM FIELDS OR ATTRIBUTES</label></td>';
				$html .= '<td class="ced_walmart_pagination"><span>' . $total_items . ' items</span>';
				$html .= '<button class="button ced_walmart_navigation" data-page="1" ' . ( ( 1 == $break_point ) ? 'disabled' : '' ) . ' ><b><<</b></button>';
				$html .= '<button class="button ced_walmart_navigation" data-page="' . esc_attr( $break_point - 1 ) . '" ' . ( ( 1 == $break_point ) ? 'disabled' : '' ) . ' ><b><</b></button><span>' . $break_point . ' of ' . $pages;
				$html .= '</span><button class="button ced_walmart_navigation" data-page="' . esc_attr( $break_point + 1 ) . '" ' . ( ( $pages == $break_point ) ? 'disabled' : '' ) . ' ><b>></b></button>';
				$html .= '<button class="button ced_walmart_navigation" data-page="' . esc_attr( $pages ) . '" ' . ( ( $pages == $break_point ) ? 'disabled' : '' ) . ' ><b>>></b></button>';
				$html .= '</td>';
				$html .= '</tr>';
			...
			$html      .= '<tr>';
			$html      .= "<td><input type='checkbox' class='ced_walmart_meta_key' value='" . esc_attr( $meta_key ) . "'></input></td>";
			...
			if ( 10 == $counter ) {
				$counter = 0;
				++$break_point;
				$html .= '<tr><td colsapn="4"><a href="" class="ced_walmart_custom_button button button-primary">Save</a></td></tr>';
				$html .= '</tbody>';
			}
		}
	} else {
		$html .= '<tr><td colspan="4" class="walmart-error">No data found. Please search the metakeys.</td></tr>';
	}
	$html .= '</table>';
	return $html;
}
