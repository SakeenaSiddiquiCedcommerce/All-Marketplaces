...
class FilterClass {
	 ...
	 * @since 1.0.0
	 */
	public function ced_tokopedia_filters_on_products( $_products, $shop_name ) {

		if ( ( ! empty( $_POST['status_sorting'] ) && isset( $_POST['status_sorting'] ) ) || ( ! empty( $_POST['pro_cat_sorting'] ) && isset( $_POST['pro_cat_sorting'] ) ) || ( ! empty( $_POST['pro_type_sorting'] ) && isset( $_POST['pro_type_sorting'] ) ) || ( ! empty( $_POST['stock_status'] ) && isset( $_POST['pro_type_sorting'] ) ) ) {
			...
			if ( ! isset( $_POST['manage_product_filters'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['manage_product_filters'] ) ), 'manage_products' ) ) {
				...
				$shop_name = isset( $_GET['shop_name'] ) ? sanitize_text_field( wp_unslash( $_GET['shop_name'] ) ) : '';
			$url           = admin_url( 'admin.php?page=ced_tokopedia&section=products-view&shop_name=' . $shop_name );
			wp_redirect( $url );
		}
	...
	public function productSearch_box( $_products, $valueTobeSearched ) {
		if ( isset( $_POST['s'] ) && ! empty( $_POST['s'] ) ) {
			...
			wp_redirect( $current_url . '&searchBy=' . $searchdata/*.'&shop_name='.$_GET['shop_name']*/ );
		} else {
			$shop_name = isset( $_GET['shop_name'] ) ? sanitize_text_field( wp_unslash( $_GET['shop_name'] ) ) : '';
			$url       = admin_url( 'admin.php?page=ced_tokopedia&section=products-view&shop_name=' . $shop_name );
			wp_redirect( $url );
