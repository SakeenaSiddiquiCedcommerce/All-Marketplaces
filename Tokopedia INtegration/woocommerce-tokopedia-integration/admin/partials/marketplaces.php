...
if ( is_array( $activeMarketplaces ) && ! empty( $activeMarketplaces ) ) {
	...
	<div class="ced-marketplaces-heading-main-wrapper">
		<div class="ced-marketplaces-heading-wrapper">
			<h2><?php esc_html_e( 'Active Marketplaces', 'woocommerce-tokopedia-integration' ); ?></h2>
		</div>
	</div>
		...
		foreach ( $activeMarketplaces as $key => $value ) {
			...
			<div class="ced-marketplace-card <?php echo esc_attr( $value['name'] ); ?>">
				<a href="<?php echo esc_attr( $url ); ?>">
					<div class="thumbnail">
						<div class="thumb-img">
							<img class="img-responsive center-block integration-icons" src="<?php echo esc_attr( $value['card_image_link'] ); ?>" height="150" width="250" alt="how to sell on vip marketplace">
						</div>
					</div>
