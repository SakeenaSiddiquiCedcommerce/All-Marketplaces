...
if (!class_exists('Class_Ced_Etsy_Import_Product')) {
	...
	class Class_Ced_Etsy_Import_Product {
		...
		public function ced_etsy_import_products( $listing_id='', $shop_name = '' ) {
			...
			$response = Ced_Etsy_WCFM_API_Request( $shop_name )->get( "application/listings/{$listing_id}", $shop_name );
			if ( isset( $response['listing_id'] ) ) {
				$this->get_listing_details( $response, $shop_name );
			} else {
				$error        = array();
		...
		private function get_listing_details( $product = array() , $shop_name ="" , $shop_id = "" ) {
			...
			$post_status_type  = $product['state']=="active"?"publish":'draft';
			$if_product_exists = get_product_id_by_shopname_and_listing_id( $shop_name , $listing_id, $post_status_type );
			...
			$inventories   = Ced_Etsy_WCFM_API_Request( $shop_name )->get( $action . 'inventory', $shop_name );
			$images        = Ced_Etsy_WCFM_API_Request( $shop_name )->get( $action . 'images', $shop_name );
			...
			if( isset( $inventories['products'] ) && count( $inventories['products'] ) > 1 ) {
				$this->create_variable_product( $product , $e_inventories , $image_details , $shop_name );
			} else {
				$this->create_simple_product( $product , $e_inventories , $image_details , $shop_name );
			}
		}

		private function create_simple_product( $product ='' , $product_details ='' ,$image_details ='', $shop_name ='' ) {

            $product_status=isset($product['state']) && $product['state']=="active"?"publish":'draft';
			$product_id = wp_insert_post( array(
				'post_title'  => $product['title'],
				'post_status' => $product_status,
				'post_type'   => 'product',
				'post_content'=> $product['description'],
				'post_author' => ced_etsy_wcfm_get_vendor_id(),
			) );
			...
			update_post_meta( $product_id, '_ced_etsy_wcfm_listing_id_' . $shop_name, $product['listing_id'] );
			update_post_meta( $product_id, '_ced_etsy_pro_state_' . $shop_name, $product['state'] );
		...
		private function create_variable_product( $product='' , $product_details='' ,$image_details='' , $shop_name='' ) {
			...
			foreach ( $etsy_variation_products as $key => $value ) {
				...
				$variations[ $key ]['product_id'] = $value['product_id'];
			}
		...
		private function insert_product_category( $product_id='' , $listing_details='' , $inventory_details='' ) {
			...
			$all_cats    = @file_get_contents( ( CED_ETSY_WCFM_DIRPATH . 'public/etsy/lib/json/categories.json' ) );
			if ( ! empty( $all_cats ) ) {
		...
		/**
		 * To insert tags with Woocommerce products.
		 *
		 */
		private function insert_product_tags( $product_id = '' , $listing_details = '' ) {
			$listing_details = !empty( $this->trans_product_tags ) ? $this->trans_product_tags : $listing_details;
		...
		private function insert_product_variations($post_id, $variations, $available_attributes) {
			$parent_qty = 0;
			foreach ( $variations as $index => $variation ){				$variation_post = array(
				...
				if (!empty( $variation['sku'] ) ) {					
					...
					update_post_meta( $variation_post_id, '_sku', str_replace( ',', '', $variation['product_id'] ) );
				}
			}
