(function( $ ) {
	 ...
	 */
	 	 var dokanajaxUrl   = ced_etsy_dokan_admin_obj.ajax_url;
	 	 var dokanajaxNonce = ced_etsy_dokan_admin_obj.ajax_nonce;
	 	 var de_shop_name   = ced_etsy_dokan_admin_obj.de_shop_name;
	 	 var parsed_response;

	 	 $( document ).on(
	 	 	'click',
	 	 	'#ced_etsy_dokan_add_account_button',
	 	 	function(){
	 	...
	 	$( document ).on(
	 		'change',
	 		'#ced_etsy_dokan_auto_upload_categories' ,
	 		function() {
	 			var categories = $( this ).val();
	 			var de_shop_name  = $( this ).data( 'shop-name' );
	 			var operation  = 'remove';
	 			if ( $( this ).is( ':checked' ) ) {
	 				operation = 'save';
	 			}
	 			$( '.ced_etsy_dokan_loader' ).show();
	 			$.ajax(
	 				{
	 					url : dokanajaxUrl,
	 					data : {
	 						ajax_nonce : dokanajaxNonce,
	 						action : 'ced_etsy_dokan_auto_upload_categories',
	 	...
	 	$( document ).on(
	 		...
	 		function(event)
	 		{
	 			...
	 			$.ajax(
	 			{
	 				...
	 				data : {
	 					ajax_nonce : dokanajaxNonce,
	 					action : 'ced_etsy_dokan_delete_account',
	 					shopid:store_id
	 				},
	 				type : 'POST',
	 				success: function(response)
	 	...
	 	$( document ).on(
	 		'hover',
	 		".ced_etsy_dokan_thumbnail li img",
	 		function(){
	 			$( '#preview-image img' ).attr( 'src',$( this ).attr( 'src' ).replace( ' ', '' ) );
	 			var cedimgSwap = [];
	 			var imgUrl     = "";
	 			$( ".ced_etsy_dokan_thumbnail li img" ).each(
	 				function(){
	 	...
	 	$( document ).on(
	 		'keyup' ,
	 		'#ced_etsy_dokan_search_product_name' ,
	 		function() {
	 			...
	 			$.ajax(
	 				{
	 					...
	 					data : {
	 						ajax_nonce : dokanajaxNonce,
	 						keyword : keyword,
	 						action : 'ced_etsy_dokan_search_product_name',
	 					},
	 					type:'POST',
	 					success : function( response ) {
	 	...
	 	$( document ).on(
	 		'click' ,
	 		'.ced_etsy_dokan_searched_product' ,
	 		function() {
	 			$( '.ced_etsy_dokan_loader' ).show();
	 	...
	 	$( document ).on(
	 		'click' ,
	 		'.ced_etsy_dokan_navigation' ,
	 		function() {
	 			$( '.ced_etsy_dokan_loader' ).show();
	 			var page_no = $( this ).data( 'page' );
	 			$( '.ced_etsy_metakey_body' ).hide();
	 			window.setTimeout( function() {$( '.ced_etsy_dokan_loader' ).hide()},500 );
	 	...
	 	$( document ).on(
	 		"click",
	 		".ced_etsy_dokan_add_account_popup_close",
	 		function(){
		...
		$( document ).on(
 			'change',
 			'#ced_etsy_carrier_name_opt',
 			function(e){
 				e.preventDefault();
 				alert('you are okay');
 				console.log('adsfasdfasdf' + $(this).val());
 				if ( ''!= $(this).val() ) {
					$( document ).find( '#ced_etsy_carrier_name' ).show();
 					$( '#ced_etsy_carrier_name').show();
 				}
 			}
 		);
	 	...
	 	$( document ).on(
	 		'click',
	 		'#ced_etsy_dokan_submit_shipment',
	 		function(){
	 			console.log('Ambikesh you were here :-');
	 			var ced_etsy_tracking_code = $( '#ced_etsy_tracking_code' ).val();
	 			var ced_etsy_carrier_name  = $( '#ced_etsy_carrier_name' ).val();
	 			console.log(ced_etsy_carrier_name);
	 			console.log(ced_etsy_tracking_code);
	 			debugger;
	 			...
	 			if (can_ajax) {
	 				...
	 				// console.log(ced_etsy_carrier_name);
	 				// console.log(ced_etsy_tracking_code);
	 				if (''==ced_etsy_carrier_name) {
	 					ced_etsy_carrier_name = $( '#ced_etsy_carrier_name_opt').val();
	 				}
	 				...
	 				// 		data : {
	 				// 			ajax_nonce : dokanajaxNonce,
	 				// 			action : 'ced_etsy_dokan_submit_shipment',
	 				...
	 				// 		success: function(response)
	 				// 	{
	 				// 			$( "#ced_etsy_dokan_submit_shipment" ).removeClass( 'disabled' );
	 	 ...
	 	 /*$( document ).on("click","#ced_etsy_authorise_account_button",function(){
	 		 $(".ced_etsy_dokan_loader").show();
	 		 var shopName=$("#ced_etsy_dokan_de_shop_name").val();
	 		 var userName=$("#ced_etsy_user_name").val();
	 			 if(shopName=="")
	 			 {
	 				 $("#ced_etsy_dokan_de_shop_name").attr('style','border:0.5px solid red');return;
	 			 }

	 			 else
	 			 {
	 				 $("#ced_etsy_dokan_de_shop_name").removeAttr('style');
	 			 }
	 		...
	 		$( document ).on(
	 			'click',
	 			'.ced_etsy_dokan_add_more_shop_section',
	 			function(){
	 		...
	 		$( document ).on(
	 			'change',
	 			'.ced_etsy_dokan_select_store_category_checkbox',
	 			function(){
	 		...
	 		$( document ).on(
	 			'click',
	 			'#ced_etsy_dokan_category_refresh_button',
	 			function()
	 			{
	 				$( '.ced_etsy_dokan_loader' ).show();
	 				$.ajax(
	 		...
	 		$( document ).on(
	 			'change',
	 			'.ced_etsy_dokan_select_category_on_add_profile',
	 			function(){
	 					...
	 					$.ajax(
	 						{
	 							...
	 							data : {
	 								ajax_nonce : dokanajaxNonce,
	 								action : 'ced_etsy_dokan_fetch_next_level_category_add_profile',
	 								level : level,
	 								name : selected_etsy_category_name,
	 								id : selected_etsy_category_id,
	 								etsy_store_id : de_shop_name
	 							},
	 							type : 'POST',
	 							success: function(response)
	 						{
	 		...
	 		$( document ).on(
	 			'change',
	 			'.ced_etsy_dokan_select_category',
	 			function(){
	 					...
	 					$.ajax(
	 						{
	 							...
	 							data : {
	 								ajax_nonce : dokanajaxNonce,
	 								action : 'ced_etsy_dokan_fetch_next_level_category',
	 								level : level,
	 								name : selected_etsy_category_name,
	 								id : selected_etsy_category_id,
	 		...
	 		$( document ).on(
	 			'click',
	 			'#ced_etsy_dokan_save_category_button',
	 			function(){
	 				...
	 				var de_shop_name             = jQuery.trim( shopname );
	 				jQuery( '.ced_etsy_dokan_select_store_category_checkbox' ).each(
	 					function(key) {
	 		...
	 		$( document ).on(
	 			'click',
	 			'#ced_esty_dokan_import_product_bulk_operation' ,
	 			function(e){
	 				e.preventDefault();
	 				...
	 				if ( operation <= 0 ) {
	 					...
	 					cedDokanimportProductsFromEtsy( etsy_import_products_id , operation );
	 				}
	 			}
	 		);
	 		...
	 		$( document ).on(
	 			...
	 			function(e) {
	 				...
	 				cedDokanimportProductsFromEtsy( listing_id_array );
	 			}
	 		);
	 	...
	 	function cedDokanimportProductsFromEtsy( etsy_import_products_id , operation ) {
	 		...
	 		$.ajax(
	 			{
	 				...
	 				data : {
	 					ajax_nonce                : dokanajaxNonce,
	 					action                    : 'ced_etsy_dokan_import_products_bulk_action',
	 				...
	 				success : function (response) {
	 					...
	 					if ( sliced_listing_id != 0 ) {
	 						cedDokanimportProductsFromEtsy( sliced_listing_id ,operation );
	 					} else {
	 						$( '.ced_etsy_dokan_loader' ).hide();
	 						response = jQuery.parseJSON( response );
	 		...
	 		$( document ).on(
	 			...
	 			function(e){
	 				...
	 				if (operation <= 0 ) {
	 					...
	 					dokanEtsyperformBulkAction( etsy_products_id,operation );
	 				}

	 			}
	 		);

	 	function dokanEtsyperformBulkAction( etsy_products_id,operation )
	 		 {
	 		if (etsy_products_id == "") {
	 				var notice = "";
	 					...
	 					success: function(response)
	 				{
	 						...
	 						if (response1 == "Shop is Not Active") {
	 							...
	 							if (remainig_products_id == "") {
	 								...
	 								dokanEtsyperformBulkAction( remainig_products_id,operation );
	 							}
	 		...
	 		$( document ).on(
	 			'click',
	 			'.ced_etsy_dokan_profiles_on_pop_up',
	 			function(){
	 				...
	 				$.ajax(
	 					{
	 						...
	 						data:{
	 							...
	 							prodId:product_id,
	 							shopid:shopid,
	 							action:'ced_etsy_dokan_profiles_on_pop_up'
	 						},
	 						type:'POST',
	 						success:function(response)
	 					{
	 									 $( ".ced_etsy_dokan_preview_product_popup_main_wrapper" ).html( response );
	 									 $( document ).find( '.ced_etsy_dokan_preview_product_popup_main_wrapper' ).addClass( 'show' );
	 						}
	 		...
	 		$( document ).on(
	 			'click',
	 			'#save_etsy_dokan_profile_through_popup',
	 			function(){
	 				$( '.ced_etsy_dokan_loader' ).show();
	 				var product_id = $( this ).attr( "data-prodId" );
	 				var shopid     = $( this ).attr( "data-shopid" );
	 				var profile_id = $( ".ced_etsy_dokan_profile_selected_on_popup" ).val();
	 				$.ajax(
	 					{
	 						...
	 						data:{
	 							...
	 							action:'save_etsy_dokan_profile_through_popup'
	 						},
	 						type:'POST',
	 						success:function(response)
	 					{
	 							...
	 							if (response == "null") {
	 								$( '.ced_etsy_dokan_loader' ).hide();
	 								$( document ).find( '.ced_etsy_dokan_preview_product_popup_main_wrapper' ).removeClass( 'show' );
	 								var notice = "";
					...
					$.ajax(
						{
							...
							data : {
								ajax_nonce : dokanajaxNonce,
								action : 'ced_etsy_dokan_get_orders',
								shopid:store_id
							},
							type : 'POST',
							success: function(response)
						{
							 $( '.ced_etsy_dokan_loader' ).hide();
							 var response  = '' != response ? jQuery.parseJSON( response ) : '';
							 var response1 = jQuery.trim( response.message );
								if (response1 == "Shop is Not Active") {
										var notice = "";
	 		...
	 		$( document ).on(
	 			'click',
	 			"#ced_etsy_dokan_update_account_status",
	 			function(){

	 				var status    = $( "#ced_etsy_account_status" ).val();
	 				var de_shop_name = $( this ).attr( "data-id" );
	 				...
	 				$.ajax(
	 					{
	 						...
	 						data : {
	 							ajax_nonce : dokanajaxNonce,
	 							action : 'ced_etsy_dokan_change_account_status',
	 							status : status,
	 							de_shop_name : de_shop_name
	 						},
	 						type : 'POST',
	 		...
	 		$( document ).on(
	 			'change',
	 			'#ced_etsy_dokan_scheduler_info',
	 			function(){

	 				if (this.checked) {
	 					  $( ".ced_etsy_dokan_scheduler_info" ).css( 'display','contents' );
	 				} else {
	 					$( ".ced_etsy_dokan_scheduler_info" ).css( 'display','none' );
	 				}
	 			}
	 		);
	 		...
	 		$( document ).on(
	 			'click' ,
	 			'.ced_etsy_delete_mapped_e_profiles' ,
	 			function(){
