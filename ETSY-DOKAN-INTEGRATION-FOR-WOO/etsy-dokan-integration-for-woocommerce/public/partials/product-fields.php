...
if ( ! class_exists( 'Ced_Etsy_Product_Fields' ) ) {
	...
	class Ced_Etsy_Product_Fields {
		 ...
		 */
		public static function get_custom_products_fields( $de_shop_name = '' ) {

			$shop_name = isset( $_GET['de_shop_name'] ) ? sanitize_text_field( wp_unslash( ced_etsy_filter( $_GET['de_shop_name'] ) ) ) : $de_shop_name;
			if ( empty( $shop_name ) ) {
				$shop_name = get_option( 'ced_etsy_dokan_de_shop_name_' . get_current_user_id() , '' );
			}
			$vendor_id             = get_current_user_id();
			$shipping_templates    = ced_etsy_dokan_get_shipping_profiles( $shop_name, $vendor_id);
			...
			<!-- <p class="form-field _umb_brand_field "> -->
				...
				<td>
					...
					if ( ! is_null( $attribute_description ) && ! empty( $attribute_description ) ) {
						ced_etsy_dokan_tool_tip( $attribute_description );
					}
		...
		*/
		public function renderDropdownHTML( $attribute_id, $attribute_name, $values, $categoryID, $productID, $marketPlace, $attribute_description = null, $indexToUse, $additionalInfo = array( 'case' => 'product' ), $is_required = false , $is_global_settings =false ) {
			$fieldName = $categoryID . '_' . $attribute_id;
			if ( 'product' == $additionalInfo['case'] ) {
				$previousValue = get_post_meta( $productID, $fieldName, true );
			} else {
				...
				<td>
					<select id="" name="<?php echo esc_attr( $fieldName . '[' . $indexToUse . ']' ); ?>" class="select short" style="">
						...
						if ( $is_global_settings ) {
							if ( '_ced_etsy_markup_type' != $attribute_id || '_ced_etsy_shop_section' != $attribute_id || '_ced_etsy_recipient' != $attribute_id ||
								'_ced_etsy_occasion' != $attribute_id 
							) {
