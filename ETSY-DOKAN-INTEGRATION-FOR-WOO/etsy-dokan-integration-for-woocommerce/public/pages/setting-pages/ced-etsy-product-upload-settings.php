...
<div class="ced_etsy_child_element">
	...
	<table class="wp-list-table ced_etsy_dokan_global_settings">
		<tbody>
			...
			$product_fields_files = CED_ETSY_DOKAN_DIRPATH . 'public/partials/product-fields.php';
			...
			if ( ! empty( $product_fields ) ) {
				...
				foreach ( $product_fields as $field_data ) {
					...
					$ced_etsy_global_data = get_option( 'ced_etsy_dokan_global_settings', array() );
					if ( ! empty( $ced_etsy_global_data ) ) {
						$data = $ced_etsy_global_data[get_current_user_id()][ $activeShop ]['product_data'];
					}
					...
					if ( '_text_input' == $field_data['type'] ) {
						...
						$productFieldInstance->renderDropdownHTML( $field_id, $label, $value_for_dropdown, $category_id, $product_id, $market_place, $description, $index_to_use, $field_value, $required , true );
					}
					echo '<td>';
