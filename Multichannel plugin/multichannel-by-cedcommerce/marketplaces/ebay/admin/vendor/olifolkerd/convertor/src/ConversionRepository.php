...
final class ConversionRepository {
	...
	public static function fromFile( string $path ): ConversionRepository {
		if ( ! file_exists( $path ) ) {
			throw new FileNotFoundException(
				"File could not be found. Given path='$path'" .
	...
	public function getAvailableConversions( string $unit ): array {
		...
		$unitConversions = array_filter(
			$this->definitions,
			function( ConversionDefinition $definition ) use ( $conversion ) {
				return $conversion->getBaseUnit() === $definition->getBaseUnit();
			}
		);

		return array_map(
			function ( ConversionDefinition $definition ) {
